...
- **Optional CORS**: browser cross-origin access can be enabled via env vars (off by default).
...
| `CORS_ENABLED` | Enable CORS middleware for browser access | off (`0`) |
| `CORS_ALLOW_ORIGINS` | Allowed origins (comma-separated or JSON array) | `*` |
| `CORS_ALLOW_ORIGIN_REGEX` | Allowed origin regex (optional) | unset |
| `CORS_ALLOW_METHODS` | Allowed methods (comma-separated or JSON array) | `*` |
| `CORS_ALLOW_HEADERS` | Allowed request headers (comma-separated or JSON array) | `*` |
| `CORS_EXPOSE_HEADERS` | Response headers exposed to browsers (comma-separated or JSON array) | unset |
| `CORS_ALLOW_CREDENTIALS` | Allow credentials (cookies/auth headers) | off (`0`) |
| `CORS_MAX_AGE` | Preflight cache max age in seconds | `600` |
